...
ull findKthLargest_XORsum(ull kth, ull start = 0) // O(D)
{
    ull retorno = start;
...
{
    ull ans = 0;
    if (valorBase <= k) ans = valorBase; // O valor base é uma possibildiade?
...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
        insertVector(a[i] ^ b[i]);

    cout << bestLEQ(k, xorAis) << endl;
}
